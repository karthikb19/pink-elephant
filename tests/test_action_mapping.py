...
def test_equivalent_white_and_black_moves_share_a_canonical_policy_index() -> None:
    ...
    assert move_to_policy_index(white_board, chess.Move.from_uci("e2e4")) == move_to_policy_index(
        black_board, chess.Move.from_uci("e7e5")
    )
    assert move_to_policy_index(white_board, chess.Move.from_uci("g1f3")) == move_to_policy_index(
