...
Positions remain pending until their game terminates. Only complete validated
games are admitted to replay, games do not cross shard boundaries, and worker
position quotas count only admitted rows. Each invocation also stores a
reconstructable per-game move record and the selected action for every replay
position.
...
Buffering positions until terminal results requires bounded worker-local memory,
but it prevents partial or invalid games from contaminating replay data.
